...
PyObject_GetAttr(PyObject *v, PyObject *name)
{
    ...
    } else if (_PyProv_Get(v)) {
        const char *owner = _PyProv_GetOwner(v);
        if (owner && owner[0]) {
                                 ...
                                 PyObject *dict, int suppress)
{
    ...
    */
    ...
    if (res != NULL) {
        /* 1) If attribute is literally called "email" and value looks like an email,
              treat it as identity: tag both the value and the container object. */
        if (PyUnicode_Check(name) && PyUnicode_Check(res)) {
            const char *n = PyUnicode_AsUTF8(name);
            const char *v = PyUnicode_AsUTF8(res);
            if (n && v && strcmp(n, "email") == 0) {
                const char *at  = strchr(v, '@');
                const char *dot = at ? strchr(at + 1, '.') : NULL;
                if (at && dot) {
                    _PyProv_TagOwned(res, v);  /* tag the email string */
                    _PyProv_TagOwned(obj, v);  /* tag the user / container */
                }
            }
        }

        /* 2) If container already has provenance and result doesn't,
              propagate owner to the attribute value. */
        if (!_PyProv_Get(res) && _PyProv_Get(obj)) {
            const char *owner = _PyProv_GetOwner(obj);
            if (owner && owner[0]) {
                _PyProv_TagOwned(res, owner);
            }
        }
    }
    /* ==== end provenance instrumentation ==== */
...
{
    PyObject *res = _PyObject_GenericGetAttrWithDict(obj, name, NULL, 0);
    if (res == NULL) {
        return NULL;
    }

    /* If container is tagged and attr is not, propagate owner to attribute */
    if (!_PyProv_Get(res) && _PyProv_Get(obj)) {
        const char *owner = _PyProv_GetOwner(obj);
        if (owner && owner[0]) {
            _PyProv_TagOwned(res, owner);
...
{
    int res = _PyObject_GenericSetAttrWithDict(obj, name, value, NULL);

    /* If the low-level set failed or this is a delete, bail */
    if (res != 0 || value == NULL) {
        return res;
    }

    /* 1) If the assigned value is already tagged, tag the container */
    if (_PyProv_Get(value)) {
        const char *owner = _PyProv_GetOwner(value);
        if (owner && owner[0]) {
            _PyProv_TagOwned(obj, owner);
        }
    }

    /* 2) If attribute is "email" and value looks like a proper email, treat as identity */
    if (PyUnicode_Check(name) && PyUnicode_Check(value)) {
        ...
        if (n && v && strcmp(n, "email") == 0) {
            ...
            const char *dot = at ? strchr(at + 1, '.') : NULL;
            if (at && dot) {
                /* Tag the value and the container with this email */
                _PyProv_TagOwned(value, v);
                _PyProv_TagOwned(obj, v);
                return res;
            }
        }
    }

    /* 3) If container is tagged, propagate its owner to any new attribute value */
    if (_PyProv_Get(obj) && value != NULL) {
        const char *owner = _PyProv_GetOwner(obj);
        if (owner && owner[0]) {
            _PyProv_TagOwned(value, owner);
        }
    }
